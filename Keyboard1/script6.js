// when double clicked, has to add the alphabet to the textArea
...
// Function to start or resume the loop from the current index
function startLoop() {
...
// Single-click and triple-click logic on control
document.querySelector('.control').addEventListener('click', () => {
    ...
    clickTimeout = setTimeout(() => {
        if (clickCount === 1 && !isDoubleClick) {
            // Single click: Start or resume the loop from where it paused
            startLoop();
        } else if (clickCount === 3) {
            // Triple click: Stop the loop completely
            stopLoop();
        }
